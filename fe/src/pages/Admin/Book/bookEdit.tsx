...
const BookEdit: React.FC = () => {
  ...
  // Load dữ liệu book + category song song
  useEffect(() => {
    if (!id) return;
    ...
    const loadData = async () => {
      try {
        ...
        const book: IBook = bookRes.data;

        // Cập nhật danh mục
        setCategories(categoryList);

        // Ép category về string nếu là object
        if (typeof book.category === "object" && book.category !== null && "_id" in book.category) {
          book.category = book.category._id;
        }

        // Reset form sau khi có danh mục
        reset(book);
      } catch (err) {
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md mt-8">
      <h2 className="text-2xl font-bold text-violet-600 mb-6 text-center">Cập Nhật Sách</h2>
